...
const request = require('request');request('https://ghibliapi.herokuapp.com/films', (error, response, body) => {   
    ...
    if (error) {
        // alt gr + è 2 fois pour écrire le ` backtick ou backquote (executé par le shell avant usage dans le code)
...
// Une fonction CallBack prend deux parametres// un parametre err qui reste vide si la fonction a bien ete executee,
...
// En utilisant les fonctions callback, ecrire une fonction qui permet de determiner le nombre
// d'occurrence d'une sous-chaıne de caractere ch dans une chaıne de caractere str.
// ch = ab
// str = abbbaaaabaaabb
...
// var searchOccurence = function (data, callback) {
//     var chocc = data.filtre;
//     if ( (data.chaine).match( new RegExp(chocc, "g") ) !== null) {
//         var nbocc = (data.chaine).match(new RegExp(chocc, "g")).length;
//         return callback(null, nbocc);
//     } else {
//         return callback(data.filtre + ' non trouvé dans ' + data.chaine);
//     };
// };
// searchOccurence(data, function (err, result) {
//     if (err)
//         console.error("erreur :" + err)
//     else
...
var searchElement2 = function (data, callback) {    
    ...
    // Si on trouve la valeur, on affiche le nombre'occurences sinon on affiche le message d'erreur
    if (pos != -1) {
        // On retourne la variable count dans le callback, nombre d'occurences
        while (pos != -1) {            
            ...
            // et un indice de début, ici pos + 1  indice à partir de la variable pos initialisée en dehors
            // de la boucle. Quand une occurence est trouve dans le chaine on incremente aussi pos,
            // ici l'indice dans la chaîne
...
promesse.then(() => console.log("test reussi")).catch(() => console.log("erreur"));
    // ouvar test = true;
var promesse = () => {
...
// affiche resultat : 5
division(10, 2).then((res) => console.log("resultat : " + res)).catch((error) => console.log(error));
// affiche erreur : division par zero
division(5, 0).then((res) => console.log("resultat : " + res)).catch((error) => console.log(error));console.log("fin");

console.log("fin");

// Remarque : les promesses s'exécutent de manière asynchrone
// ASYNC / AWAIT
